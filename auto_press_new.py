...
def init_logging():
    logger_config = {
        ...
        "formatters": {
            "standard": {
                "format": "[%(asctime)s] [%(levelname)-8s]  [%(module)-10s:%(lineno)3d]  [%(thread)d] %(message)s"
            }
        },
        "handlers": {
            "console": {
    ...
    for work_id in work_ids:
        ...
        for schedule in schedules[::-1]:
            if (schedule["partition_str"]>"2019-09-00" and schedule["status"]=="SUCCESS") or (schedule["cron_type"]=="STREAM" and schedule["status"]=="RUNNING"):
                ...
                logger.info((schedule["work_id"], schedule["id"], schedule["cron_type"]))
                break
...
def generate_job_and_outputtable(schedules_list):
    ...
    for work_id, schedule_id, cron_type in schedules_list:
        ...
        logger.info("schedule_url: "+schedule_url)
        for job_info in job_infos["execute_DAG"]:
            try:
                if job_info.get("job_info") \
                    ...
                    job_list.append((work_id, config["job_id"], job_info["name"], job_info["job_info"]["configs"]["command"], "1536M", "0.3", owner, cron_type))
                    ...
                    logger.info("  hdfs: /tmp/ting.wu/k8s_press/{}.json".format(config["job_id"]))
...
def copy_job_output_dayu_table(outtable_list):
    ...
    for outtable in outtable_list:
        try:
            ...
            failed_table[str(outtable['dayu_id'])] = outtable
    if failed_table:
        # plogger.info(failed_table)
        logger.info("create dayu table error, {}".format(len(failed_table)))
...
def save_log(pod_name, folder_name, status):
    ...
    with open("{}-{}/{}.log".format(folder_name, status, pod_name),'w+') as log_op:
        log_op.write(log)
    logger.info("save {}-{}/{}.log".format(folder_name, status, pod_name))
...
def save_yaml(yaml_dict, folder_name, status):
    ...
    config_yaml.close()
    logger.info("save: {}-{}/{}.yaml".format(folder_name, status, yaml_dict['metadata']['name']))
...
def append_log(pod_name, folder_name, status):
    ...
    with open("{}-{}/{}.log".format(folder_name, status, pod_name),'w+') as log_op:
        log_op.write(log)
    logger.info("append history/{}-{}/{}.log".format(folder_name, status, pod_name))

def generate_job_folder_name_map(job_list):
    job_folder_name_map = {}
    for work_id,job_id, job_name, command, mem, core,owner,cron_type in job_list:
        folder_name = "history/{}-{}-{}-{}-{}".format(work_id,job_name, owner, job_id, cron_type)
...
def submit_pod(v1):
    ...
    logger.info("submit pod: {}".format(driver_pod["metadata"]["name"]))
...
def submit_confmap_service(v1):
    ...
    logger.info("submit confmap: {}".format(confmap["metadata"]["name"]))

    with open("press/service.yaml", "r") as op:
        service = yaml.load(op)
    v1.create_namespaced_service("dev", service)
    logger.info("submit service: {}".format(service["metadata"]["name"]))
...
def main():
    limit = 10
    init_logging()
    logger.info("***"*10+"generate work id"+"***"*10)
    work_ids = generate_work(limit)
    logger.info(len(work_ids))
    logger.info("***"*10+"generate_schedule id"+"***"*10)
    schedules_list = generate_schedule(work_ids)
    logger.info(len(schedules_list))
    logger.info("***"*10+"generate_job_and_outputtable"+"***"*10)
    job_list, outtable_list = generate_job_and_outputtable(schedules_list)
    logger.info("job_list: "+str(len(job_list)))
    logger.info("outtable_list: "+str(len(outtable_list)))
    logger.info("***"*10+"copy_job_output_dayu_table"+"***"*10)
    ...
    logger.info("***"*10+"press_job_on_k8s"+"***"*10)
